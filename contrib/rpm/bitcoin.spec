...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8392
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8393
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18392
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8392
	%{_sbindir}/semanage port -d -p tcp 8393
	%{_sbindir}/semanage port -d -p tcp 18392
